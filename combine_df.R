...
reg08 <- cces08 %>% 
  ...
  select(meet, sign, work, donate, age, pid7, acts, attend = V217, interest = V244, educ = V213, race = V211, income = V246, weight = V201) %>% 
  mutate(year = 2008) %>% 
  as_tibble()
...
reg10 <- cces10 %>% 
  ...
  mutate(acts = meet + sign + work + donate) %>% 
  select(meet, sign, work, donate, age, pid7, acts, attend = V217, interest = V244, educ = V213, race = V211, income = V246, weight = V101) %>% 
  mutate(year = 2010) %>% 
  as_tibble()
...
reg12 <- cces12 %>% 
  ...
  mutate(donate = car::recode(CC417a_4, "1=1; 2=0")) %>% 
  mutate(acts = meet + sign + work + donate) %>% 
  select(meet, sign, work, donate, age, pid7, acts, attend = pew_churatd, interest = newsint, educ, race, income = faminc, weight = weight_vv) %>% 
  mutate(year = 2012) %>% 
  as_tibble()
...
reg14 <- cces14 %>% 
  ...
  mutate(donate = car::recode(CC417a_4, "1=1; 2=0")) %>% 
  mutate(acts = meet + sign + work + donate) %>% 
  select(meet, sign, work, donate, age, pid7, acts, attend = pew_churatd, interest = newsint, educ, race, income = faminc, weight = weight) %>% 
  mutate(year = 2014) %>% 
  as_tibble()
...
reg16 <- cces16 %>% 
  ...
  mutate(acts = meet + sign + work + donate) %>% 
  select(meet, sign, work, donate, age, pid7, acts, attend = pew_churatd, interest = newsint, educ, race, income = faminc, weight = commonweight_vv) %>% 
  mutate(year = 2016) %>% 
  as_tibble()
...
reg18 <- cces18 %>% 
  ...
  mutate(acts = meet + sign + work + donate) %>% 
  select(meet, sign, work, donate, age, pid7, acts, attend = pew_churatd, interest = newsint, educ, race, income = faminc_new, weight = commonweight) %>% 
  mutate(year = 2018) %>% 
  as_tibble() 
